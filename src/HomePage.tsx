...
export const HomePage = () => {
  const [entryData, loading, error] = useCollectionData<entry>(
    query(collection(db, 'entries'), orderBy('createdAt', 'asc')).withConverter(
      entryConverter
    )
  );
  ...
  return (
    <div className="w-full flex flex-col items-center gap-2">
      ...
      {user &&
        entryData?.map((ele: entry, idx: number) => {
          return (
            <div
              key={ele.id}
              className="flex flex-row items-center justify-center w-min gap-4 p-4 border border-2 border-black"
            >
              <p>{ele.id}</p>
              <p>{ele.rating}</p>
            </div>
